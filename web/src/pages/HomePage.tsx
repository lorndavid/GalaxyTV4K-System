...
export const HomePage: React.FC = () => {
  ...
  // Short name: localized appropriately
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-white via-white to-slate-50/70 dark:from-dark-surface dark:via-dark-surface dark:to-dark-elevated border border-slate-200/80 dark:border-dark-border shadow-[0_8px_30px_rgb(0,0,0,0.05)] dark:shadow-none p-5 sm:p-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            {isFullStudyDay && !isCheckedIn ? (
              <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60">
                {isKhmer ? 'វេនរៀនសូត្រ' : 'Study Day'}
              </span>
            ) : isMorningChineseStudy ? (
              <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60">
                {isKhmer ? 'វេនរសៀល' : 'Afternoon Shift'}
              </span>
            ) : null}
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-dark-elevated/70 border border-slate-100 dark:border-dark-border/80 relative overflow-hidden">
            ...
            <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium block mt-1">
              {isCompletedToday
                ...
                ? isKhmer ? 'វេនរៀនសូត្រ' : 'Study Day'
                : isMorningChineseStudy
                ? isKhmer ? 'រង់ចាំម៉ោង ១២:០០' : 'Opens at 12:00 PM'
        ...
        {/* Primary Punch Call to Action Button */}
        {isCompletedToday ? (
          <div className="w-full py-3.5 px-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2.5 bg-slate-100 dark:bg-dark-elevated text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-dark-border">
          ...
          <Link
            to="/scan"
            className="w-full py-3.5 px-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2.5 transition-all duration-200 active:scale-[0.98] shadow-md bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white shadow-orange-500/25"
          >
            <Fingerprint className="w-5 h-5 animate-pulse" />
            <span>{t('attendance.punchOutBtn', 'ចុច Check-Out ចេញពីធ្វើការ')}</span>
          </Link>
        ) : isMorningChineseStudy ? (
          <div className="w-full py-3.5 px-4 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 bg-indigo-50/80 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60 shadow-xs">
            <Clock3 className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
            <span>
              {isKhmer
                ? 'Check-In វេនរសៀល (បើកម៉ោង ១២:០០ ថ្ងៃត្រង់)'
                : 'Afternoon Check-In (Opens at 12:00 PM)'}
            </span>
          </div>
        ) : isFullStudyDay ? (
          <div className="w-full py-3.5 px-4 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 bg-indigo-50/80 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60 shadow-xs">
            ...
            <span>
              {isKhmer
                ? 'ថ្ងៃនេះជាវេនរៀនសូត្រ (មិនតម្រូវឱ្យស្កេនចូល)'
          ...
          <Link
            to="/scan"
            className="w-full py-3.5 px-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2.5 transition-all duration-200 active:scale-[0.98] shadow-md bg-gradient-to-r from-brand-600 to-blue-600 hover:from-brand-700 hover:to-blue-700 text-white shadow-brand-500/30 ring-2 ring-brand-500/20"
          >
            <Fingerprint className="w-5 h-5 animate-pulse" />
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          ...
          <div className="p-3 rounded-2xl bg-slate-50 dark:bg-dark-elevated/60 border border-slate-100 dark:border-dark-border/60">
            <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium block truncate">
              {isAfternoonShift
                ? isKhmer ? 'រៀនចិនព្រឹក' : 'Morning Study'
          ...
          <div className="p-3 rounded-2xl bg-slate-50 dark:bg-dark-elevated/60 border border-slate-100 dark:border-dark-border/60">
            <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium block truncate">
              {isAfternoonShift
                ? isKhmer ? 'ចូលធ្វើការ' : 'Afternoon In'
                : t('home.lunchBreak', 'សម្រាកថ្ងៃត្រង់')}
            </span>
            <span className="font-bold text-slate-900 dark:text-slate-100 font-mono text-xs mt-1 block">
              {isAfternoonShift ? '12:00 – 13:00' : '11:30 – 13:00'}
            </span>
          </div>
