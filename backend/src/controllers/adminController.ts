    ...
    const result = await prisma.$transaction(async (tx) => {
      const employee = await tx.employee.create({
        data: {
          ...
          shiftType: shiftType?.trim() || 'STANDARD',
          checkInStartTime: checkInStartTime?.trim() || '07:30',
          checkInDeadline: checkInDeadline?.trim() || '07:30',
          workEndTime: workEndTime?.trim() || '17:30',
          studyClassInfo: studyClassInfo?.trim() || null,
